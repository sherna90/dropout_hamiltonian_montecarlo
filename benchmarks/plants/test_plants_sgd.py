...
def train_model():
    sampler=inference.sgd(model,start_p,step_size=eta)
    par,loss=sampler.fit(epochs=epochs,batch_size=batch_size,gamma=0.9,X_train=X_train,y_train=y_train,verbose=True)
    print('SGD, time:',time.time()-start_time)
    loss=pd.DataFrame(loss)
    loss.to_csv('loss.csv',sep=',',header=False)
    with open('results/sgd/model.pkl','wb') as handler:
        pickle.dump(par,handler)

def test_model():
    with open('results/sgd/model.pkl','rb') as handler:
        samples=pickle.load(handler)
    predict_samples=[]
    par={var:samples[var] for var in samples.keys()}
    y_pred=model.predict(par,X_test,prob=True,batchsize=32)
    y_pred=y_pred.reshape(-1, y_pred.shape[-1])
    with h5py.File('results/sgd/output.hdf5', 'w') as f:
        f["predict_samples"] = y_pred

def evaluate_model():
    file=h5py.File('results/sgd/output.hdf5', 'r')
    y_pred=file['predict_samples'][:,:]
...
def test_outofsample():
    ...
    y_pred=y_pred.reshape(-1, y_pred.shape[-1])
    conf_mat=confusion_matrix(y_test[:].argmax(axis=1), y_pred.argmax(axis=1),labels=np.arange(38))[9:11,:]
    report=classification_report(y_test[:].argmax(axis=1), y_pred.argmax(axis=1),output_dict=True)
    print(report)
    with open('sgd_class.pkl','wb') as handler:
        pickle.dump(conf_mat,handler)
    sns.heatmap(conf_mat,linewidths=.5,cmap="Blues",robust=True,yticklabels=labels[9:11],xticklabels=labels,cbar=False,square=True)
    plt.title('SGD', fontsize=16)
    plt.savefig("sgd_class.pdf", bbox_inches="tight")
    plt.show()
    with h5py.File('results/sgd/outofsample_output.hdf5', 'w') as f:
        f.create_dataset('predict_samples', data=np.asarray(y_pred)) 
...
def evaluate_model_outofsample():
    ...
    tmat[1,10]=1
    y_test=y_test.dot(tmat)
    file=h5py.File('results/sgd/outofsample_output.hdf5', 'r')
    correct=[]
    incorrect=[]
    tp=[]
    fp=[]
    ...
    results=pd.DataFrame({'class':tp+fp,'predicted label':np.concatenate([label0,label1]),'probability':np.concatenate([correct,incorrect])})
    ...
    print('incorrect non-healthy, mean : %5.2f : , std : %5.2f'%(np.mean(non_healthy_incorrect),np.std(non_healthy_incorrect)))
    ...
    plt.title('SGD', fontsize=16)
    plt.savefig("sgd_out.pdf", bbox_inches="tight")
    plt.show()
