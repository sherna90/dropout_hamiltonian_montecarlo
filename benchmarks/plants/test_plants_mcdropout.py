...
def test_model():
    with open('results/mcdropout/model.pkl','rb') as handler:
    ...
    with h5py.File('results/mcdropout/output.hdf5', 'w') as f:
...
def test_outofsample():
    ...
    for i in range(epochs):
        print('prediction : {0}'.format(i))
        y_pred=model.predict_stochastic(par,X_test,p=0.5,prob=True,batchsize=y_test.shape[0])
        y_pred=y_pred.reshape(-1, y_pred.shape[-1])
    ...
    results=pd.DataFrame({'precision':precision,'recall':recall,'f1':fscore})
    print(results.describe())
    with open('mcdropout_class.pkl','wb') as handler:
    ...
    plt.title('MC dropout', fontsize=16)
    plt.savefig("mcdropout_class.pdf", bbox_inches="tight")
    plt.show()
    with h5py.File('results/mcdropout/outofsample_output.hdf5', 'w') as f:
        f.create_dataset('predict_samples', data=np.asarray(predict_samples)) 
...
def evaluate_model_outofsample():
    ...
    tmat[1,9]=1
    y_test=y_test.dot(tmat)
    index=(plants_test['labels'][:,0]==1)
    file=h5py.File('results/mcdropout/outofsample_output.hdf5', 'r')
    epochs=file['predict_samples'].shape[0]
    correct=[]
    incorrect=[]
    tp=[]
    ...
    plt.title('MC dropout', fontsize=16)
    plt.savefig("mcdropout_out.pdf", bbox_inches="tight")
    plt.show()
