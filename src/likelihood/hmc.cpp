...
void Hamiltonian_MC::warmup(){
	if (this->init_hmc){
		...
		tools.writeToCSVfile("warmup_mean.csv", mu); //
		tools.writeToCSVfile("warmup_cov.csv", this->multivariate_gaussian.getCov()); //
		tools.writeToCSVfile("warmup_invcov.csv", this->inv_cov); //
	}
}
