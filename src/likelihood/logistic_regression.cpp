...
void LogisticRegression::init(MatrixXd &_X,VectorXd &_Y,double _lambda, bool _normalization,bool _standardization,bool _with_bias){
	...
	if (this->with_bias) this->bias=1.0*0.1;
 	else this->bias=0.0;
	this->weights =tools.random_generator(dim)*0.1;
	this->grad_bias = 0.0;
}
...
double LogisticRegression::logLikelihood(){
	ArrayXd y_slice_array=this->y_slice.array();
	ArrayXd phi_array=this->phi.array();
	ArrayXd log_likelihood = y_slice_array*phi_array.log() + ((1.0-y_slice_array)*(1-phi_array).log());
