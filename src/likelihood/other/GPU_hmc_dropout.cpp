...
void Mask_GPU_Hamiltonian_MC::init(MatrixXd &_X, VectorXd &_Y, double _lambda, int _warmup_iterations, int _iterations, double _step_size, int _num_step, double _mask_rate, bool _normalization, bool _standarization, double _path_length){
	this->lambda=_lambda;
...
VectorXd Mask_GPU_Hamiltonian_MC::gradient(VectorXd &W, VectorXd &mask){
...
double Mask_GPU_Hamiltonian_MC::logPosterior(VectorXd &W,  VectorXd &mask, bool precompute){
	double logPost = 0.0;
...
void Mask_GPU_Hamiltonian_MC::run(bool warmup_flag, bool for_predict){
	...
	if (this->init_hmc){	
		...
		while (n < this->iterations){
			...
			if (!warmup_flag and !for_predict){
				...
				//mask = tools.random_binomial(1, 0.5, this->dim -1);
		...
		if (!warmup_flag and !for_predict){
			double mean = mask_matrix.rowwise().sum().mean();
			cout << "Mask Mean: "<< mean << endl;	
			double std = sqrt((mask_matrix.rowwise().sum().array() - mean).array().square().sum() / (mask_matrix.rows()));
			cout << "Mask STD: " << std << endl;
...
VectorXd Mask_GPU_Hamiltonian_MC::predict(MatrixXd &X_test, bool prob, int samples, bool simulation){
