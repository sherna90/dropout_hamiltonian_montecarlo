...
public:
	C_utils();
	MatrixXd VtoM(VectorXd &W, int _n_classes, int _dim);
	VectorXd MtoV(MatrixXd &W);
	int ReverseInt (int i);
	void read_Mnist(string filename, MatrixXd& data);
	void read_Mnist_Label(string filename, VectorXd& labels);
	double unif(double min, double max);
	VectorXd random_generator(int dimension);
	Mat RGBtoLAB(Mat &frame);
	double random_uniform();
	VectorXd random_uniform_generator(int dimension);
	VectorXd random_binomial(int n, VectorXd prob, int dim);
	VectorXd random_binomial(int n, double prob, int dim);
    ...
    VectorXd argMin(MatrixXd data, bool row = true);
    VectorXd argMax(MatrixXd data, bool row = true);
    VectorXd matrixDot(MatrixXd &A, VectorXd &x);
    VectorXd sign(VectorXd &x);
    VectorXd vecMax(double value, VectorXd &vec);
    int getIndex(VectorXd &x, double value);
    MatrixXd binarizeLabels(VectorXd &x, VectorXd& classes);
    VectorXd getClasses(VectorXd &x);
